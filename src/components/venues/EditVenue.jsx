...
import {useEditVenue, useGetOneVenue} from "../../hooks/useVenues.js";
...
export default function EditVenue() {
    const {id} = useParams();
    const {venue, isLoading} = useGetOneVenue(id);
    const edit = useEditVenue();
    const navigate = useNavigate();
    const [error, setError] = useState('');

    const editVenueHandler = async ({ name, image, address, phone, email, description}) => {
        ...
        try {
            await edit(id, name, image, address, phone, email, description);
            navigate(`/venues/${id}`);
        } catch (err) {
            if (err.message) {
    ...
    const {values, changeHandler, submitHandler} =
        useForm(Object.assign(initialValues, venue), editVenueHandler);
